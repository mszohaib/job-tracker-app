...
function App() {
  ...
  const startEdit = (job) => {
    ...
    const appDate = job.applicationDate || job.application_date;
    setEditApplicationDate(appDate ? String(appDate).slice(0, 10) : "");
  ...
  const handleLogin = async (event) => {
    ...
    if (!response.ok) {
      setAuthError(result.error||result.message||"login Failed!")
      return;
    }
  ...
  const handleRegister = async () => {
    ...
    if (!response.ok) {
      setAuthError(result.error || result.message|| "Registration failed");
      return;
    }
  ...
  // Adding the authenticating ui if there is token
  if (!token) {
    return (
        ...
        <div className="min-h-screen flex items-center justify-center">
          <div className="bg-white p-6 rounded-xl shadow-md space-y-4 w-80">
            <h2 className="text-xl font-bold text-center">
             {/* Putting the error ui nicely to users */}
             {authError && (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        {/* The date applied and the notes if there is any */}
                        {(job.applicationDate || job.application_date) && (
                            <p className="text-xs text-slate-500 mt-1">
                              Applied: {new Date(job.applicationDate || job.application_date).toLocaleDateString("en-US", { 
                                year: "numeric", 
                                month: "short", 
                                day: "numeric"  })}
                                </p>
